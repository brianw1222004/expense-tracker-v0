...
All state lives in `App.js`; everything under `src/` is presentational or a pure helper. Navigation is hand-rolled (no react-navigation): a `tab` state (`dashboard | list`) drives two kept-mounted tab screens (the inactive one is `display: 'none'` so scroll state survives switching — dismiss the keyboard on tab change), an `addOpen` state drives the add-expense popup (`AddExpenseModal`, also kept mounted via `display: 'none'` so a half-typed expense survives dismissal), and an `overlay` state (`settings | compare | null`) drives two Modal screens.
