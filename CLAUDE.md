...
- **Cache-first.** On sign-in or launch the per-user AsyncStorage cache renders immediately, then `syncWithServer()` reconciles with Supabase in the background and on each return to foreground. A `dataUser` state records which user the in-memory data belongs to, and the save-to-cache effects are gated on `dataUser === userId` so an account switch can't write one user's data under another's cache key. They also skip a value `cachedRef` records as already in the cache, so a load isn't written straight back.
...
- **`src/budget.js`** — pure budget math. Allocation: `budgetAmountPercent`, `totalAllocatedBudget`/`remainingBudget` (the "Allocated / Remaining" readout), `hasUsableOverallBudget`, `clampBudgetRatio`, and clamping so category budgets never exceed the overall (`clampCategoryBudgetAmount`/`maxBudgetForCategory`, with `fitAllocatedBudgetsToOverall` proportionally shrinking allocations when the overall drops). `maxBudgetForCategory` is the overall minus every OTHER regular category's allocation — it excludes the edited category exactly once, so an edit reuses its own capacity and an already over-allocated pool repairs the edited share rather than preserving it. `categoryBudgetUpdate` is the single entry point both budget UIs go through: it clamps, applies the creation minimum (5% of the overall, any positive amount when none is set) AFTER clamping, and returns `{amount, categoryBudgets, minimum, valid}` — zero or blank DELETES the key rather than storing a zero. External categories bypass the ceiling by passing `external: true`. Progress bars: `budgetZoneTone` (green / orange ≥85% / red over) and `categoryBarState({spent, budget, colors})` → `{basis: 'budget'|'none', ratio, fillPct, tone, over}` — always spent-of-budget, with `basis: 'none'` when no budget is set so callers label the slot "No budget". Both the Dashboard and Insight rows share this rule; don't re-derive it.
- **`src/categorySettings.js`** — `saveCategorySettings(settings, category, isNew)`, the one place a category's metadata and its budget are applied together. Both `addCustomCategory` and `updateCustomCategory` in App.js go through it, so the Insight add/edit modal and the Budget editor sheet can't drift apart on allocation rules. It upserts on id, strips `budget`/`_editing` off the stored object, and returns `settings` UNCHANGED when a creation fails its minimum — `patchSyncedSettings` treats that identity result as a no-op and skips the version bump and sync push.
...
- **`AccountScreen`** — the bottom Modal sheet behind the floating account button: account info and sign-out, a "Delete all data" danger action, and the language + theme switchers. The danger action runs `deleteAllData` (`src/deleteAllData.js`, which owns all the logic and is tested directly; App.js wires it as `handleDeleteAllData`). Cloud users get it too, with its own confirm copy (`acct.deleteBodyCloud`): sync is suspended (`suspendSync`, so no lane pushes mid-wipe), `wipeServerData` waits for any in-flight push, deletes every row for the user in `split_expenses`/`groups`/`expenses`/`settings`, then **counts what remains** (RLS can refuse a delete without an error) — a missing table counts as empty, and an absent count is unverified rather than empty. The retired `income` table is wiped best-effort and deliberately NOT verified: failing the whole delete over a dead feature's rows, after every live table was already emptied, is the worse outcome. Both modes then drain pending cache writes and remove AND verify the pending-op lanes (`clearQueues(userId, { strict: true })`) and tracker keys (`clearUserStorage(userId, { strict: true })`) before resetting in-memory state; cloud users are signed out last, and a sign-out that fails (supabase-js RESOLVES with `{ error }`) is reported via `acct.deleteSignOutFailed` instead of counting as success. On any failure nothing is reset, sync resumes with its queue intact, and the copy says which state the data is in — `acct.deleteNothingRemovedBody` only while that is still provable, `acct.deleteFailedBody` (some data may already be gone) once deleting has begun. Every step is bounded AND cancellable — the wipe aborts its requests at a deadline, the drain gives up before removing anything, and a 30s backstop unlocks the sheet and aborts a shared signal that `wipeServerData`, `clearQueues` and `clearUserStorage` each check before they remove anything, so a step that overran can't delete after the user was told it failed. Two limits are accepted by design: the server deletes are separate requests with no transaction around them, and suspension binds only this process — another signed-in device holding queued ops can still push rows back. The reset marks its empty values as already cached (`cachedRef` in App.js), so the save effects don't re-create the removed keys. `deleteDataGuard` (a ref) blocks re-entry; `accountLocked` (state) is that same window made visible and is what disables the ×, the settings rows and the delete row. Removing the Supabase auth record itself still needs a server function.
...
- **`AddCategoryModal`** — add/edit a custom category: name, **required** monthly budget (min 5% of the overall budget, any positive amount when none is set) that is also clamped to the unallocated remainder via `categoryBudgetUpdate`, paginated icon grid, preset colors plus a hue slider, and an external switch. When the remainder can't cover the creation minimum, Save is disabled and `cats.budgetAllocationConflict` says why; when the ceiling cuts a typed figure the hint warns with `cats.budgetClamped`. App.js's `addCustomCategory`/`updateCustomCategory` save the budget into `settings.categoryBudgets` and strip the `budget` field off the returned object.
