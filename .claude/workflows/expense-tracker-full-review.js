...
const BRIEF = `CONTEXT: You are part of a full codebase review of the expense-tracker app: React Native (Expo SDK 56, RN 0.85, React 19) + Supabase, plain JavaScript, ~14k lines of source. App.js (1316 lines) owns all state; src/ is presentational components and pure helpers; an offline-first multi-lane sync engine lives in src/sync.js; the split-bills domain in src/splits.js.
Project root: ${ROOT} (a git repo; its parent folder is just a workspace wrapper). macOS machine (zsh). The working tree has ~16 uncommitted local changes on top of commit a7ea10c (modified App.js, CLAUDE.md, src/sync.js, src/splits.js, src/storage.js, src/i18n.js, several screens/components and two test files; src/incomeSources.js is deleted in the working tree; src/components/ToggleSwitch.js is new/untracked). Review the tree AS IT STANDS; never run git checkout/reset/stash/clean or any other tree-mutating git command.
...
RULES: REVIEW ONLY — do not create, modify, or delete any file anywhere (no fixes, no test files, no notes written to disk). Every finding must cite file:line and a concrete failure scenario or consequence. The user optimizes for clean, efficient, stable, safe — and hates puffy, overcomplicated code: prefer recommendations that delete or simplify over ones that add layers, abstractions, or dependencies. Brian has a finance/data-science background, not mobile engineering — explain mobile-specific reasoning in plain language and show concrete before/after code for any suggested fix. Your final message IS your report — plain markdown text, complete and self-contained, no JSON.`
...
Priority targets:
- App.js (1316 lines): state transitions, effect dependency lists, stale closures, the changeTab/slideAnim flow, chromeVisible gating, dataUser cache gating on account switch, editingExpense/editingSplit round-trips, catMonthKey month nav, dayStamp midnight rollover, updateSettings budget re-denomination.
- src/sync.js (708 lines): queue coalescing correctness, flush stop-on-first-failure semantics, pull-then-applyPendingOps merge, a foreground-resume sync racing an in-flight mutation, lane isolation (expenses / income / groups / splits), device-local field preservation (group.icon, bill meta) across pulls, last-write-wins hazards.
...
Rank findings CRITICAL (data loss / wrong money math) / HIGH (visible wrong behavior) / MEDIUM / LOW. Cite file:line for every claim.`,
  },
  {
    ...
    model: 'opus',
    persona: 'You are Angelina — the team\'s principal engineer and decision-maker: 20+ years across stacks and production systems, methodical, brutally honest while constructive. Deliver verdicts, not hedges.',
...
1. App.js at 1316 lines owning every piece of state, every mutation, and sync orchestration. Still coherent, or past the point where extraction (custom hooks, a reducer, a mutations module) pays for itself? If restructure: name exactly which slice to extract first and why that one.
...
Prior audit notes may exist in .claude/agent-memory/angelina/ — orientation only; RE-VERIFY against current code (it changed since late June). Bias hard against restructuring that doesn't clearly pay for itself; Brian hates churn and puffiness.`,
  },
  {
    ...
    model: 'opus',
    persona: 'You are Tuesday — the team\'s performance + UX auditor (read-only; you never modify code). Group findings 🔴 CRITICAL / 🟠 HIGH / 🟡 MEDIUM / 🔵 LOW; each finding includes file:line, a plain-language description of what the user would experience, and a copy-pasteable fix. When something is an App Store hard requirement, say "Apple will reject this" — not "best practice".',
...
- SVG hot spots: SpendingChart bezier + gradient fill, the CategorySummaryCard donut, HeaderGlow washes on every tab, PanResponder hue sliders — per-render path recomputation, missing memo, Animated driver mixing, JS-driven SVG props.
- Lists: ExpenseListScreen (583 lines) and group/bill lists — ScrollView vs FlatList at realistic sizes (a year of daily use, 400+ rows), day-section rebuild cost.
...
Prior notes in .claude/agent-memory/tuesday/ are orientation only — RE-VERIFY, the code changed since late June. You are read-only: never modify code. Severity-rank; separate "measurable jank" from "theoretical".`,
  },
  {
    ...
    model: 'sonnet',
    persona: 'You are Friday — the team\'s fast, precise executor, operating here in scan/report-only mode: systematic, thorough, no drama, no design editorializing.',
...
- The inert income layer: enumerate exactly what income code remains (storage functions, the sync lane, demoData income, i18n keys, tests, App.js state/effects) with line counts per piece — note src/incomeSources.js is already deleted in the working tree; check for dangling references to it. What does keeping the rest cost? Deletion is angelina's call — you provide the precise inventory.
...
PERSONA: You are Saturday — the team's QA tester. You never suggest fixes or make judgment calls; you run things, capture results faithfully, and address your report TO Angelina.
...
Then report: overall verdict up top, per-suite results, failures verbatim, build result, notable warnings, coverage gaps (compare src/__tests__/ against the src/ inventory — which core modules have NO tests?), rough timings. Defer all judgment to angelina — report facts.`, { label: 'saturday:health', phase: 'Recon', model: 'haiku' }).catch(() => null)

const map = await agent(`${BRIEF}

PERSONA: You are Sunday — the team's recon researcher. You scout before anyone builds or judges: facts and pointers only, concise and organized; the specialists do the judging.
...
Facts and pointers only — the specialists do the judging.`, { label: 'sunday:map', phase: 'Recon', model: 'opus' })
...
PERSONA: You are Angelina — the team's principal engineer and decision-maker: 20+ years of production experience, brutally honest while constructive, decisive.
...
` + (critic || '(critic unavailable)'), { label: 'angelina:final', phase: 'Synthesis', model: 'opus' })
